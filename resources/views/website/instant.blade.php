...
@section('title', 'تسليم فوري - Dasa\'s Cake')
                        ...
                        <div class="row">
                                ...
                                <div class="image-gallery">
                                    ...
                                    @if($allImages->count() > 0)
                                        ...
                                        <img src="{{ asset('storage/' . $allImages->first()) }}"
                                             ...
                                             id="mainImageInstant{{ $product->id }}">
                                            ...
                                            <div class="gallery-thumbnails">
                                                @foreach($allImages as $index => $image)
                                                    <img src="{{ asset('storage/' . $image) }}"
                                                         ...
                                                         onclick="changeImageInstant{{ $product->id }}('{{ asset('storage/' . $image) }}', this)">
                                                @endforeach
                                            </div>
                                        @endif
                                    @else
                            ...
                            <div class="col-md-6">
                                ...
                                <div class="stock-info">
                                    ...
                                    <span>متوفر: {{ $product->total_stock }} قطعة</span>
                                </div>
                                ...
                                <form action="{{ route('cart.add') }}" method="POST">
                                    ...
                                    <input type="hidden" name="order_type" value="instant">
    ...
    @empty
        <div class="col-12">
            <div class="d-flex flex-column align-items-center justify-content-center text-center py-5">
                <img src="{{asset('empty-fawre.png')}}" width="300" alt="" class="mb-3">
                <h4 class="mt-3 text-muted">لا توجد منتجات متوفرة حالياً</h4>
                <p class="text-muted">تفقد صفحة الحجز لطلب منتجاتك</p>
                <a href="{{ route('reservation') }}" class="btn btn-secondary mt-2">
                    صفحة الحجز
                </a>
            </div>
        </div>
...
window.changeImageInstant{{ $product->id }} = function(imageSrc, thumbnail) {
    // Update main image
    const mainImage = document.getElementById('mainImageInstant{{ $product->id }}');
    if (mainImage) {
